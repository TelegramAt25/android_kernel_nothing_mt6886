...
/*********************************************************************/
static int sc8562_set_ovpgate_dg(struct sc8562 *sc, int threshold)
{
	int ret;
	u8 val;

	if (threshold == 50)
		val = SC8562_OVPGATE_ON_DG_50MS;
	else
		val = SC8562_OVPGATE_ON_DG_220MS;

	val <<= SC8562_OVPGATE_ON_DG_SHIFT;
	ret = sc8562_update_bits(sc, SC8562_REG_03, SC8562_OVPGATE_ON_DG_MASK, val);
	return ret;
}
/*********************************************************************/
...
static int sc8562_parse_dt(struct sc8562 *sc, struct device *dev)
{
	...
	ret = of_property_read_u32(np, "sc,sc8562,ovpgate-dg-threshold",
				&sc->cfg->ovpgate_dg_th);
	if (ret) {
		sc_err("failed to read ovpgate_dg_th\n");
		return ret;
	}
...
static int sc8562_init_protection(struct sc8562 *sc)
{
	...
	ret = sc8562_set_ovpgate_dg(sc, sc->cfg->ovpgate_dg_th);
	sc_info("set ovpgate dg %d %s\n", sc->cfg->ovpgate_dg_th, !ret ? "successfully" : "failed");
