	...
	case TCP_NOTIFY_SINK_VBUS:
		...
		if (usb_psy && nci->info) {
			...
			if (sink_mv && sink_ma) {
				...
				/*
					sink_mv = 0 ,sink_ma = 0 don't disable power_path
					sink_mv = 5v, sink_ma = 0 disable power_path
				*/
				if (sink_mv) {
					val.intval = 1;
					power_supply_set_property(usb_psy,
	...
	const char __user *buffer, size_t count, loff_t *pos)
{
	...
	if (!sscanf(desc, "%d", &area_id)) {
		pr_info("%s: write area_id value failed!\n", __func__);
		return -EINVAL;
	}
	nci->area_id = area_id;
	g_nt_area_id = area_id;
	pr_info("%s: area_id = %d\n", __func__, nci->area_id);
	return count;
}
