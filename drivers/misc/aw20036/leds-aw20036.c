...
/* Ensure that the brightness value conversion of strip 3 is correct*/
#define BRIGHTNESS_TRANSFORM(X)	(((X) * 133) + 51) / 100;
				...
				unsigned char reg_addr, u8 length, unsigned char * reg_data)
{
	...
	while (cnt < AW_I2C_RETRIES) {
		ret = i2c_transfer(aw20036->i2c->adapter, &msg, 1);
		if (ret < 0) {
...
static int aw20036_rgbcolor_config(struct aw20036 *aw20036)
{
	...
	2, 2, 2, 2, 2, 2, 2, 2, 16,2, 2, 2,
	2, 2, 2, 2, 2, 2, 2, 2, 2, 2A, 2, 2
	*/
	unsigned char aw20036_rgb_color_cfg_black[36] ={
		0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04,
		0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x2C, 0x04, 0x04, 0x04,
		0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x3F, 0x04, 0x04
	};
	unsigned char aw20036_rgb_color_cfg_white[36] ={
		0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02,
		0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x16, 0x02, 0x02, 0x02,
		0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x2A, 0x02, 0x02
	};
			...
			struct device_attribute *attr, char *buf)
{
	...
	len +=
		snprintf(buf + len, PAGE_SIZE - len, "current cfg = %s\n",
			aw20036_cfg_name[aw20036->effect]);
#elif defined(AW20036_ARRAY_CONFIG)
	for (i = 0; i < sizeof(aw20036_cfg_array) / sizeof(struct aw20036_cfg); i++) {
		len +=
				...
				const char *buf, size_t len)
{
	...
	if (strncmp(dev_color, "BLACK", 5) == 0) {
		/* Change the brightness of LED33 to 1.33-fold */
		if (data_2[19] > 191)
			data_2[19] = 255;
		else
			data_2[19] = BRIGHTNESS_TRANSFORM(data_2[19]);
	}
	aw20036_reg_page_cfg(aw20036, AW20036_REG_PAGE2);
	aw20036_i2c_write_block(aw20036, 0x00, 14, data_1); /*led(0-13)*/
				...
				const char *buf, size_t len)
{
	...
	if (strncmp(dev_color, "BLACK", 5) == 0) {
		...
		if (data[33] > 191)
			...
			data[33] = BRIGHTNESS_TRANSFORM(data[33]);
	}
				...
				const char *buf, size_t len)
{
	...
	const char *p;
	int val=0;
	unsigned int frame_brightness[26];
	int frame_num =0;
	unsigned int i, num;
	unsigned char brightness_1[14] = {0};
	...
	} else if (frame_num == 24) {
		int led_C1[24] = {0, 12, 24, 1, 13, 25, 2, 14, 26, 3, 15, 27, 4, 16, 28, 5, 17, 29, 6, 18, 30, 7, 19, 31};
		for (i = 0; i < 24; i++) {
			num = led_C1[i];
			if (num < 14) {
				brightness_1[num] = frame_brightness[i];
	...
	if (strncmp(dev_color, "BLACK", 5) == 0) {
		/* Change the brightness of LED33 to 1.33-fold */
		if (brightness_2[19] > 191)
			brightness_2[19] = 255;
		else
			brightness_2[19] = BRIGHTNESS_TRANSFORM(brightness_2[19]);
	}
	aw20036_reg_page_cfg(aw20036, AW20036_REG_PAGE2);
	aw20036_i2c_write_block(aw20036, 0x00, 14, brightness_1); /*led(0-13)*/
				...
				const char *buf, size_t len)
{
	...
	if (strncmp(dev_color, "BLACK", 5) == 0) {
		...
		aw20036_imax_cfg(aw20036, 0x05);//default is white
	}
...
static void aw20036_lamps_flowing_work(struct work_struct *work)
{
	...
	unsigned int num;
	int led0[24] = {0, 12, 24, 1, 13, 25, 2, 14, 26, 3, 15, 27, 4, 16, 28, 5, 17, 29, 6, 18, 30, 7, 19, 31};
	...
	for (num = 0; num < 24; num++) {
		ret = aw20036_i2c_write(aw20036, led0[num], brightness);
		if (ret < 0) {
			goto end;
		}
		if (!((num + 1) % 3))
	...
	//clear
	for (num = 0; num < 24; num++) {
		ret = aw20036_i2c_write(aw20036, led0[num], led_off);
		if (ret < 0) {
			goto end;
		}
		if (!((num + 1) % 3))
				...
				const char *buf, size_t len)
{
	...
	if (sscanf(buf, "%d %d %d %d %d",
		...
		ret = aw20036_i2c_write(aw20036, led2, round_leds_br);
		if (ret < 0) {
			return ret;
		}
#if 0
...
static void aw20036_leds_effect_work(struct work_struct *work)
{
	...
	do {
		if (aw20036->curr_buf->status == MMAP_BUF_DATA_VALID) {
			pr_info("%s BUF_DATA[%d] copy from FW length/%d secNum %d\n", __func__, aw20036->curr_buf->bit, aw20036->curr_buf->length, aw20036->sec_num);
			if (aw20036->sec_num == 5) {
			...
			}else if (aw20036->sec_num == 26) {
				for (i = 0; i < aw20036->curr_buf->length; i += 26) {
					start = ktime_get();
					for (j = 0; j < 26; j++) {
						num = led_26[j];
						if (num < 14) {
				...
				if ((aw20036->curr_buf->length < 494) || (aw20036->stream_mode == 0)) {
					pr_info("%s break 3\n", __func__);
					break;
				}
			}
