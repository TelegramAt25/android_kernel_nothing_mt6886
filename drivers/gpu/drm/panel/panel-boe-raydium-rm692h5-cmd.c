...
#endif
#include "include/panel-boe-raydium-rm692h5-cmd.h"
/* i2c control start */
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	lcm_dcs_write_seq_static(ctx,0x00,0xD0);// 50=OD off D0=OD on
	...
	lcm_dcs_write_seq_static(ctx,0x1A,0x40);
	lcm_dcs_write_seq_static(ctx,0x3B,0x54);
			      ...
			      dcs_write_gce cb, void *handle, bool en)
{
	...
	mtk_panel_proc_hbm(g_ctx->hbm_stat);
	ctx->hbm_en = en;
	ctx->hbm_wait = true;
                   ...
                   struct device_attribute *attr, char *buf)
{
	...
	lcm_set_page(6, 0xFE, 0x20);  //FE-ED
	val1 =  get_reg_val(1, 0xf8);

	val2 =  get_reg_val(1, 0xf9);
...
int sys_node_init(void)
{
	...
	if (ret < 0) {
		printk("[%s] sysfs_create_group ui_status failed\n",__func__);
		return -1;
	}
...
static struct mipi_dsi_driver rm692h5_boe_rm_lcm_driver = {
	.probe = lcm_probe,
	.remove = lcm_remove,
	.driver = {
		.name = "panel-rm692h5-boe-rm-cmd",
		.owner = THIS_MODULE,
		.of_match_table = lcm_of_match,
	},
};

static int __init rm692h5_boe_rm_lcm_driver_init(void)
{
	int ret = 0;

	pr_notice("%s+\n", __func__);
	mtk_panel_lock();
	ret = mipi_dsi_driver_register(&rm692h5_boe_rm_lcm_driver);
	if (ret < 0)
...
static void __exit rm692h5_boe_rm_lcm_driver_exit(void)
{
	pr_notice("%s+\n", __func__);
	mtk_panel_lock();
	mipi_dsi_driver_unregister(&rm692h5_boe_rm_lcm_driver);
	mtk_panel_unlock();
	pr_notice("%s-\n", __func__);
}
module_init(rm692h5_boe_rm_lcm_driver_init);
module_exit(rm692h5_boe_rm_lcm_driver_exit);
