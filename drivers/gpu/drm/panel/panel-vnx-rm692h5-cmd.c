...
#endif
#include "include/panel-vnx-rm692h5-cmd.h"
/* i2c control start */
...
lcm_dcs_write_seq_static(ctx,0x96,0x00);
	...
	lcm_dcs_write_seq_static(ctx,0xFE,0x00);
	lcm_dcs_write_seq_static(ctx,0x2F,0x06);  //60HZ
//	lcm_dcs_write_seq_static(ctx,0xFE,0x00);
//	lcm_dcs_write_seq_static(ctx,0xFA,0x01);
	...
	void *handle, unsigned int level)
{
	...
	if (lcm_now_state) {  /*60nit  4094  30nit  2048 5nit  341*/
		if (level >= 1200) {
			bl_tb0[1] = 0x0f;
			bl_tb0[2] = 0xfe;
...
{
	if (hwinfo != NULL) {
		strcpy(hwinfo->chip,"RM692H5");
		strcpy(hwinfo->vendor,"Raydium");
		strcpy(hwinfo->more,"1084*2412");
	}
}
...
static struct mipi_dsi_driver rm692h5_lcm_driver = {
	.probe = lcm_probe,
	.remove = lcm_remove,
	.driver = {
		.name = "panel-rm692h5-vnx-cmd",
		.owner = THIS_MODULE,
		.of_match_table = lcm_of_match,
	},
};

static int __init rm692h5_lcm_driver_init(void)
{
	int ret = 0;

	pr_notice("%s+\n", __func__);
	mtk_panel_lock();
	ret = mipi_dsi_driver_register(&rm692h5_lcm_driver);
	if (ret < 0)
		pr_notice("%s, Failed to register rm692h5_lcm_driver: %d\n",
			__func__, ret);
...
static void __exit rm692h5_lcm_driver_exit(void)
{
	pr_notice("%s+\n", __func__);
	mtk_panel_lock();
	mipi_dsi_driver_unregister(&rm692h5_lcm_driver);
	mtk_panel_unlock();
	pr_notice("%s-\n", __func__);
}
module_init(rm692h5_lcm_driver_init);
module_exit(rm692h5_lcm_driver_exit);

MODULE_AUTHOR("Yi-Lun Wang <Yi-Lun.Wang@mediatek.com>");
MODULE_DESCRIPTION("rm692h5 VNX CMD LCD Panel Driver");
